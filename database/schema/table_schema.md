# Table Schema
The schema documented here reflects the current layout of `database/schema/00_create_database.sql` for the EnergyOperationsPlatform database.

The file defines the database, creates the core tables, and then applies the foreign key constraints separately via `ALTER TABLE` statements. This approach keeps the DDL order-safe and easier to maintain.

> Note: The tables in this markdown mirror the SQL script directly, including the final `ALTER TABLE` section for foreign key relationships.
...
The script applies foreign keys after all tables are created.
...
ALTER TABLE invoice_line_items ADD CONSTRAINT fk_invoice_line_items_contract_billing_item FOREIGN KEY (contract_billing_item_id) REFERENCES contract_billable_items(contract_billable_item_id);
